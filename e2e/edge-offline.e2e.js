describe('Edge Cases - Offline & Permission Interruptions', () => {
    ...
    describe('App State Interruptions', () => {
        it('should handle app backgrounding during login', async () => {
            ...
            await element(by.id('email-input')).replaceText('john@gmail.com');
            await element(by.id('password-input')).replaceText('Pass');

            // Send app to background (platform-specific behavior)
            if (device.getPlatform() === 'ios') {
                // iOS: Use device.sendToHome() for backgrounding
                await device.sendToHome();
            } else {
                // Android: Also use sendToHome() but behavior might differ
                await device.sendToHome();
            }
            ...
            // Complete login with platform-specific keyboard handling
            await element(by.id('password-input')).replaceText('Password123!');
        ...
        it('should handle device orientation changes', async () => {
            ...
            // Rotate device to landscape (iOS and Android support)
            ...
            // Try login in landscape with platform-specific input
            await element(by.id('email-input')).replaceText('john@gmail.com');
            await element(by.id('password-input')).replaceText('Password123!');

            // Dismiss keyboard (orientation change might affect keyboard)
            if (device.getPlatform() === 'android') {
                await device.pressBack();
            } else if (device.getPlatform() === 'ios') {
                await element(by.id('password-input')).tapReturnKey();
    ...
    describe('Memory & Performance Edge Cases', () => {
        ...
        it('should handle multiple login attempts', async () => {
            ...
            for (let i = 0; i < 3; i++) {
                ...
                await element(by.id('email-input')).replaceText(`wrong${i}@email.com`);
                await element(by.id('password-input')).replaceText('WrongPassword!');

                // Platform-specific keyboard dismiss
                if (device.getPlatform() === 'android') {
                    await device.pressBack();
                } else if (device.getPlatform() === 'ios') {
                    await element(by.id('password-input')).tapReturnKey();
            ...
            // Final keyboard dismiss before successful login
            if (device.getPlatform() === 'android') {
                await device.pressBack();
            } else if (device.getPlatform() === 'ios') {
                await element(by.id('password-input')).tapReturnKey();
